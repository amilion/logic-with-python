...
def test_polish(debug=False):
    ...
    assert Formula('~', Formula('&', Formula(
        'p'), Formula('q7'))).polish() == '~&pq7'
    if debug:
        print("Testing polish of formula '~->&pq7|p1p12' (in infix: '~((p&q7)->(p1|p12))')")
    assert Formula('~', Formula('->', Formula('&', Formula('p'), Formula('q7')),
                   Formula('|', Formula('p1'), Formula('p12')))).polish() == '~->&pq7|p1p12'
...
def test_repr_all_operators(debug=False):
    ...
    if debug:
        print("Testing representation of formula '(p<->~p)'")
    assert str(Formula('<->', Formula('p'),
               Formula('~', Formula('p')))) == '(p<->~p)'
    if debug:
        print("Testing representation of formula '~(p~&q7)'")
    assert str(Formula('~', Formula(
        '-&', Formula('p'), Formula('q7')))) == '~(p-&q7)'
    if debug:
        print("Testing representation of formula '(~(p+q)<->(~q<->~p))'")
